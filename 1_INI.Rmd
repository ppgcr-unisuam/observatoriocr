...
has.sucupira <- length(sucupira.list) == 0
```
...
has.dados.cadastrais <-
  !is.error(try(read_excel(paste0("PPG/Dados Cadastrais.xlsx"), sheet = 1),
                silent = TRUE)
  )
```
...
```{r try-discentes}
has.discentes <-
  !is.error(try(read_excel(paste0("PPG/Discentes.xlsx"), sheet = 1),
                silent = TRUE)
  )
```
...
has.docentes.posdocs <-
  !is.error(try(read_excel(paste0("PPG/Docentes e Pós-Docs.xlsx"), sheet = 1),
                silent = TRUE)
  )
```
...
```{r try-inventario}
has.inventario <-
  !is.error(try(read_excel(paste0("PPG/Inventário.xlsx"), sheet = 1),
                silent = TRUE)
  )
```
...
has.artigos.completos <-
  !is.error(try(read_excel(paste0("PPG/Produção.xlsx"), sheet = "PI.completos"), silent = TRUE)
  )

has.artigos.resumos <-
  !is.error(try(read_excel(paste0("PPG/Produção.xlsx"), sheet = "PI.resumos"), silent = TRUE)
  )

has.preprints <-
  !is.error(try(read_excel(paste0("PPG/Produção.xlsx"), sheet = "PI.preprints"), silent = TRUE)
  )

has.capitulos <-
  !is.error(try(read_excel(paste0("PPG/Produção.xlsx"), sheet = "PI.livros"), silent = TRUE)
  )

has.eventos <-
  !is.error(try(read_excel(paste0("PPG/Produção.xlsx"), sheet = "PI.eventos"), silent = TRUE)
  )

has.producao <-
  any(has.artigos.completos, has.artigos.resumos, has.preprints, has.eventos)
```
