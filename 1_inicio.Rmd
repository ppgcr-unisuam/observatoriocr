...
```{r install-update-packages, echo = FALSE}
...
clean_dois <- ifelse(
  ...
  doi_cleaner(data.frame(DOI = produtos.detalhes.sucupira$`Valor do Item de Detalhamento`))$DOI,
...
```{r altmetric-doi-sucupira, cache = TRUE, cache.path = "cache/altmetric-doi-sucupira/", eval = has.metricas.doi.sucupira}
# iterate item-wise
for (item in 1:nrow(produtos.sucupira)) {
  # match ID da Produção entre produtos.sucupira e produtos.detalhes.sucupira
  matches <- which(
  ...
  # get DOI values
  dois <- data.frame(DOI = unique(na.omit(produtos.detalhes.sucupira$DOI[matches]))[1])
...
# rename column
names(produtos.detalhes.sucupira)[names(produtos.detalhes.sucupira) == "score"] <- "altmetric_score"
```

### **CrossRef (Open Access, Citations)**

```{r crossref-doi-sucupira, cache = TRUE, cache.path = "cache/crossref-doi-sucupira/", eval = has.metricas.doi.sucupira}
...
```{r scopus-issn-sucupira, cache = TRUE, cache.path = "cache/scopus-issn-sucupira/", eval = has.metricas.doi.sucupira}
# continue from previous chunk

# get unique ISSN
issn.unique <- data.frame(ISSN = as.character(unique(na.omit(produtos.detalhes.sucupira$ISSN))))
...
for (item in 1:nrow(issn.unique)) {
  ...
  # get ISSN values
  df_scopus <- data.frame(issn = issn.unique$ISSN[item])
...
```{r plumx-doi-sucupira, cache = TRUE, cache.path = "cache/plumx-doi-sucupira/", eval = has.metricas.doi.sucupira}
# add to data.frame
```

### **Dimensions (score)**

```{r dimensions-doi-sucupira, cache = TRUE, cache.path = "cache/dimensions-doi-sucupira/", eval = has.metricas.doi.sucupira}
# add to data.frame
```

## **Check DOI produção PPG**

```{r metricas-doi-ppg, cache = TRUE, cache.path = "cache/metricas-doi-ppg/", eval = FALSE}
...
produtos.detalhes.sucupira <-
  produtos.detalhes.sucupira %>%
  dplyr::filter(
    !is.na(`ID da Produção`) | !is.na(`Ano da Produção`)
  )
