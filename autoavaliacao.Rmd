...
`r if(all(exists("out.1"), render_document, has.sucupira.files)){paste(knitr::knit(text = out.1), collapse = '\n\n')}`
...
# convert to horizonta table
coerencia.tbl <- matrix(NA, nrow = 0, ncol = length(unique(coerencia$`Área de Concentração`))*length(unique(coerencia$`Linha de Pesquisa`)))
colnames(coerencia.tbl) <- paste0("AC: ", rep(unique(coerencia$`Área de Concentração`), each = length(unique(coerencia$`Linha de Pesquisa`))), " - ", "LP: ", rep(unique(coerencia$`Linha de Pesquisa`), times = length(unique(coerencia$`Área de Concentração`))))
...
table.doc.long <- do.call(rbind, sucupira)
...
<!--- knit those table chunk statements --> 
`r if(all(!sjmisc::is_empty(lista.kw), render_document, has.sucupira.files)){paste(knitr::knit(text = out.2), collapse = '\n\n')}`
...
sucupira.df.bibl <- data.frame(do.call(dplyr::bind_rows, sucupira))
...
sucupira.df.details <- data.frame(do.call(dplyr::bind_rows, sucupira))
...
# filter `Nome do Autor` = docentes.quadrienio
# sucupira.df.autor <- sucupira.df.bibl[sucupira.df.bibl$`Nome do Autor` %in% docentes.quadrienio, ]
...
colnames(table.docente.long) <- c("Ano", "Docente")
...
if (knitr::is_latex_output() && !sjmisc::is_empty(table.insercao.gt)) {
  ...
  kable_table <- gtsummary::as_kable_extra(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = ifelse(knitr::is_html_output(), "", "Docentes permanentes com insercão na graduação"),
  ) %>%
    kableExtra::kable_styling(
      bootstrap_options = c("basic", "hover", "condensed", "responsive"),
      latex_options = c("basic", "repeat_header", "HOLD_position"),
...
```{r site, eval = all(render_document, has.dados.cadastrais), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H', out.width = "100%"}
dados.cadastrais <-
  readxl::read_excel("PPG/Dados Cadastrais.xlsx",
                     sheet = "Endereço",
                     col_types = c("text"))
site <- which("URL:" == dados.cadastrais$`Dados do Programa`)
URL.site <- dados.cadastrais$Endereço[site]
...
if(!sjmisc::is_empty(URL.site)){
  ...
  a$Page$navigate(URL.site)
  Sys.sleep(1)
  screenshot(a, "site.png")
  if(knitr::is_html_output()) {
...
if(!sjmisc::is_empty(URL.obs)){
  ...
  b$Page$navigate(URL.obs)
  Sys.sleep(1)
  screenshot(b, "observatorio.png")
  if(knitr::is_html_output()) {
...
```{r 3.3.2 assessorias-de-ONGs-empresas-agencias-de-fomento-revistas-científicas-ou-orgaos-de-governo, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
if (knitr::is_html_output()) {
  cat('##### *Assessoria*')
  cat('\n\n')
}
...
```{r 3.3.2 lideranca-de-sociedades-e-ou-de-orgãos-científicos-culturais-profissionais-sociais-governamentais, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
if (knitr::is_html_output()) {
  cat('##### *Liderança*')
  cat('\n\n')
}
...
if (knitr::is_html_output() && !sjmisc::is_empty(member.all)) {
  ...
  knitr::kable(
    ...
    full_width = TRUE,
    position = "center"
  )  %>% kableExtra::column_spec(column = 1:dim(member.all)[2], width = paste0(c(0.10,0.10,0.25,0.30,0.15,0.10)*(21-3-3), "cm")) %>%
  kableExtra::column_spec(column = 1, bold = TRUE) %>%
  kableExtra::row_spec(row = 0, bold = TRUE)
} else {
  # use tinytable for LaTeX output
...
if (knitr::is_html_output() && !sjmisc::is_empty(services.all)) {
  ...
  knitr::kable(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = "Assessorias de ONGs, empresas, agências de fomento, revistas científicas ou órgaos de governo"
  ) %>% kableExtra::kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = TRUE,
    position = "center"
  ...
  tinytable::tt(
    ...
    style = "bootstrap",
    align = "l",
    caption = ifelse(knitr::is_html_output(), "", "Assessorias de ONGs, empresas, agências de fomento, revistas científicas ou órgaos de governo")
  ) %>%
    tinytable::theme_tt("multipage", rowhead = 1) %>%
