  ...
  if (knitr::is_latex_output() && !sjmisc::is_empty(sucupira.perfil.disc)) {
    ...
    kable_table <- gtsummary::as_kable_extra(
      ...
      kableExtra::kable_styling(
        ...
        full_width = TRUE,
        position = "center"
      ) %>% kableExtra::column_spec(column = 1:(1 + length(unique(sucupira.perfil.disc$Anos))*nlevels(sucupira.perfil.disc$"Nível Discente")), width = paste0(c(0.55, rep(0.45/(length(unique(sucupira.perfil.disc$Anos))*nlevels(sucupira.perfil.disc$"Nível Discente")), times = length(unique(sucupira.perfil.disc$Anos))*nlevels(sucupira.perfil.disc$"Nível Discente")))*(29-3-3), "cm"))
    print(kable_table)
  }
