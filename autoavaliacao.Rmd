...
# Escape characters in table data & % $ # _ { }
if(knitr::is_latex_output()) {
  try(peer.review.all <- apply(peer.review.all, 2, function(x) gsub("\\&", "\\\\&", x)), silent = TRUE)
  try(peer.review.all <- apply(peer.review.all, 2, function(x) gsub("\\%", "\\\\%", x)), silent = TRUE)
  try(peer.review.all <- apply(peer.review.all, 2, function(x) gsub("\\$", "\\\\$", x)), silent = TRUE)
  try(peer.review.all <- apply(peer.review.all, 2, function(x) gsub("\\#", "\\\\#", x)), silent = TRUE)
  try(peer.review.all <- apply(peer.review.all, 2, function(x) gsub("\\_", "\\\\_", x)), silent = TRUE)
  try(peer.review.all <- apply(peer.review.all, 2, function(x) gsub("\\{", "\\\\{", x)), silent = TRUE)
  try(peer.review.all <- apply(peer.review.all, 2, function(x) gsub("\\}", "\\\\}", x)), silent = TRUE)
}
