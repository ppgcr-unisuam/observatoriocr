...
##### **Descrição** {#descricao .tabset}

```{r 1-1-1-AC, eval = has.dados.cadastrais}
area.raw <-
...
print(
  kable(area.ativas, align = c("l", rep(
...
```{r 1-1-1-LP, eval = has.dados.cadastrais}
linhas.raw <-
  readxl::read_excel("PPG/Dados Cadastrais.xlsx",
             sheet = "LP",
             col_types = c("text"))
...
```{r 1-1-1-PP-DP, eval = has.sucupira.files}
table.projetos <-
  data.frame(
...
print(
  kable(table.projetos, align = c("l", rep(
...
```{r 1-1-1-coerencia-ac, eval = has.sucupira.files}
# AC-LP-PP
...
# recode
sucupira$'Data de Início da Linha' <-
  substr(as.Date(sucupira$'Data de Início da Linha', format = "%d/%m/%Y"), 1, 4)
sucupira$'Data de Fim da Linha' <-
  substr(as.Date(sucupira$'Data de Fim da Linha', format = "%d/%m/%Y"),
         1, 4)
sucupira$'Data de Início do Projeto' <-
  substr(as.Date(sucupira$'Data de Início do Projeto', format = "%d/%m/%Y"), 1, 4)
sucupira$'Data da Situação' <-
  substr(as.Date(sucupira$'Data da Situação', format = "%d/%m/%Y"), 1, 4)
sucupira$'Data da Situação'[is.na(sucupira$'Data da Situação')] <-
  format(Sys.Date(), "%Y")

# filter duplicated rows (changes in details will we exhibited)
sucupira <- sucupira[!duplicated(sucupira), ]
sucupira <- sucupira[order(sucupira$"Data de Início do Projeto"),]

data <- dplyr::select(sucupira, c("Área de Concentração", "Linha de Pesquisa", "Nome do Projeto de Pesquisa", "Data de Início do Projeto", "Situação", "Data da Situação"))

table.ac <- data.frame(matrix(NA, nrow = length(unique(data$`Área de Concentração`)), ncol = length(quadrienal.vigente)))
rownames(table.ac) <- NULL
table.ac <- cbind(unique(data$`Área de Concentração`), table.ac)
colnames(table.ac) <- c("Área de Concentração", as.character(quadrienal.vigente))
for (i in 1:dim(table.ac)[1]) {
  for (j in 1:length(quadrienal.vigente)) {
    table.ac[i, j + 1] <-
      try(data$`Área de Concentração`[match(table.ac$`Área de Concentração`[i], data$`Área de Concentração`)]
          , silent = TRUE)
  }
}

table.ac.long <- tidyr::gather(table.ac[, -1], "Ano", "Área de Concentração", colnames(table.ac)[-1], factor_key=TRUE)

table.ac.long <- rbind(
  table.ac.long,
  data.frame(
    "Ano" = rep(paste0(min(quadrienal.vigente), "-", max(quadrienal.vigente)), times = dim(table.ac)[1]),
...
table.ac.gt <-
  ...
  remove_row_type(variables = c('Área de Concentração'), 
                  type = "level", level_value = ("Áreas")) %>%
  modify_column_hide(columns = paste0("stat_", as.character(length(quadrienal.vigente) + 1)))
```

```{r 1-1-1-coerencia-lp, eval = has.sucupira.files}
# AC-LP-PP
sheet <- "Linhas de Pesquisa - Projetos"
source("Scripts/read-xlsx-sucupira.R", local = knitr::knit_global())
...
table.lp <- data.frame(matrix(NA, nrow = length(unique(data$`Linha de Pesquisa`)), ncol = length(quadrienal.vigente)))
rownames(table.lp) <- NULL
table.lp <- cbind(unique(data$`Linha de Pesquisa`), table.lp)
colnames(table.lp) <- c("Linha de Pesquisa", as.character(quadrienal.vigente))
...
table.lp.long <- tidyr::gather(table.lp[, -1], "Ano", "Linha de Pesquisa", colnames(table.lp)[-1], factor_key=TRUE)

table.lp.long <- rbind(
  table.lp.long,
  data.frame(
    "Ano" = rep(paste0(min(quadrienal.vigente), "-", max(quadrienal.vigente)), times = dim(table.lp)[1]),
...
table.lp.gt <-
  ...
  remove_row_type(variables = c('Linha de Pesquisa'), 
                  type = "level", level_value = ("Linhas")) %>%
  modify_column_hide(columns = paste0("stat_", as.character(length(quadrienal.vigente) + 1)))
```

```{r 1-1-1-coerencia-pp, eval = has.sucupira.files}
# AC-LP-PP
sheet <- "Linhas de Pesquisa - Projetos"
source("Scripts/read-xlsx-sucupira.R", local = knitr::knit_global())
...
table.pp <- data.frame(matrix(NA, nrow = length(unique(data$`Nome do Projeto de Pesquisa`)), ncol = length(quadrienal.vigente)))
rownames(table.pp) <- NULL
table.pp <- cbind(unique(data$`Nome do Projeto de Pesquisa`), table.pp)
colnames(table.pp) <- c("Nome do Projeto de Pesquisa", as.character(quadrienal.vigente))
for (i in 1:dim(table.pp)[1]) {
  for (j in 1:length(quadrienal.vigente)) {
    table.pp[i, j + 1] <-
      try(data$`Nome do Projeto de Pesquisa`[match(table.pp$`Nome do Projeto de Pesquisa`[i], data$`Nome do Projeto de Pesquisa`)]
          , silent = TRUE)
  }
}

table.pp.long <- tidyr::gather(table.pp[, -1], "Ano", "Nome do Projeto de Pesquisa", colnames(table.pp)[-1], factor_key=TRUE)

table.pp.long <- rbind(
  table.pp.long,
  data.frame(
    "Ano" = rep(paste0(min(quadrienal.vigente), "-", max(quadrienal.vigente)), times = dim(table.pp)[1]),
...
table.pp.gt <-
  ...
  remove_row_type(variables = c('Nome do Projeto de Pesquisa'), 
                  type = "level", level_value = ("Projetos")) %>%
  modify_column_hide(columns = paste0("stat_", as.character(length(quadrienal.vigente) + 1)))
```

```{r ac-lp-pp, eval = has.sucupira.files}
...
```{r ac-detalhes, eval = has.sucupira.files}
options(knitr.kable.NA = '')
print(
  kable(table.ac, align = c("l", rep(
    "c", ncol(table.ac) - 2
  ), "r")) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = T,
...
```{r lp-detalhes, eval = has.sucupira.files}
options(knitr.kable.NA = '')
print(
  kable(table.lp, align = c("l", rep(
    "c", ncol(table.lp) - 2
  ), "r")) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = T,
...
```{r pp-detalhes, eval = has.sucupira.files}
options(knitr.kable.NA = '')
print(
  kable(table.pp, align = c("l", rep(
    "c", ncol(table.pp) - 2
  ), "r")) %>% kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = T,
...
```{r 1-1-1-docentes, eval = has.sucupira.files}
...
```{r dimensao-corpo-docente-detalhes, eval = has.sucupira.files}
options(knitr.kable.NA = '')
print(
  kable(table.doc.1, align = c("l", rep(
...
```{r vinculacao-corpo-docente-detalhes, eval = has.sucupira.files}
options(knitr.kable.NA = '')
print(
  kable(table.doc.2, align = c("l", rep(
...
```{r regime-corpo-docente-detalhes, eval = has.sucupira.files}
options(knitr.kable.NA = '')
print(
  kable(table.doc.3, align = c("l", rep(
...
```{r carga-horaria-corpo-docente-detalhes, eval = has.sucupira.files}
options(knitr.kable.NA = '')
print(
  kable(table.doc.4, align = c("l", rep(
...
```{r coerencia-corpo-docente-detalhes, eval = has.sucupira.files}
options(knitr.kable.NA = '')
print(
  kable(table.doc.5, align = c("l", rep(
