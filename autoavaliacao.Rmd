...
```{r 3-3-2-bancas-de-trabalho-de-conclusao-de-curso, eval = all(render_document, has.lattes.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
...
# filter by quadrienio vigente
bancas_summary <- bancas_summary[bancas_summary$Ano %in% quadrienal.vigente, ]

# sort by Ano decreasing
bancas_summary <- bancas_summary[order(bancas_summary$Ano, decreasing = TRUE), ]
rownames(bancas_summary) <- NULL

# print tables
if (knitr::is_html_output() && !sjmisc::is_empty(bancas_summary)) {
  # use kable for HTML output
  knitr::kable(
    bancas_summary,
    align = "l",
    ...
    booktabs = TRUE,
    linesep = "",
    caption = "Bancas examinadoras de trabalho de conclusão de curso"
  ) %>% kableExtra::kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = TRUE,
    position = "center"
  )  %>% kableExtra::column_spec(column = 1:dim(bancas_summary)[2], width = paste0(c(0.05,0.15,0.15,0.20,0.25,0.15,0.30,0.10)*(29-3-3), "cm")) %>%
    kableExtra::column_spec(column = 1, bold = TRUE) %>%
    kableExtra::row_spec(row = 0, bold = TRUE) %>%
    kableExtra::footnote(
      general_title = "<strong>Fontes</strong>: ",
      ...
      escape = FALSE)
} else if (knitr::is_latex_output() && !sjmisc::is_empty(bancas_summary)) {
  # use tinytable for LaTeX output
  bancas_summary <- data.frame(bancas_summary, check.names = FALSE)
  options(tinytable_print_output = "latex")
  tinytable::tt(
    bancas_summary,
    width = c(0.05,0.15,0.15,0.20,0.25,0.15,0.30,0.10)*(29-3-3),
    align = "l",
    caption = ifelse(knitr::is_html_output(), "", "Bancas examinadoras de trabalho de conclusão de curso"),
    notes = list(
      "\\textbf{Fontes}: \\textbf{\\href{https://lattes.cnpq.br/}{Plataforma Lattes}}"
    ),
    escape = TRUE
  ) %>%
    tinytable::theme_latex(
    ...
    tinytable::style_tt(i = 0, j = 1:ncol(bancas_summary), bold = TRUE) %>%
    tinytable::style_tt(i = 1:nrow(bancas_summary), j = 1, bold = TRUE)
}
