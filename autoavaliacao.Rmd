...
```{r 2-1-2-data, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
# Produções - Autores
...
sucupira.df.bib <- data.frame(do.call(dplyr::bind_rows, sucupira), check.names = FALSE)
names(sucupira.df.bib) <- names(sucupira[[as.character(max(as.numeric(quadrienal.vigente)))]])

# filter by Subtipo = Trabalho Em Anais
sucupira.df.bib <- sucupira.df.bib %>% 
...
# filter by Produção vinculada a TCC concluído
sucupira.df.bib <- sucupira.df.bib %>% 
  dplyr::filter(`Produção vinculada a TCC concluído` == "Sim")

# get unique ID da Produção
ID <- unique(sucupira.df.bib$`ID da Produção`)
...
# select sucupira.df.bib with matching ID
sucupira.df.bib <- sucupira.df.bib[sucupira.df.bib$`ID da Produção` %in% ID,]
...
sucupira.df.bib$`Tipo de Produção` <- tools::toTitleCase(tolower(sucupira.df.bib$`Tipo de Produção`))
sucupira.df.bib$`Subtipo de Produção` <- tools::toTitleCase(tolower(sucupira.df.bib$`Subtipo de Produção`))

sucupira.df.bib$`Tipo de Produção`[is.na(sucupira.df.bib$`Tipo de Produção`)] <- "Indefinido"
sucupira.df.bib$`Subtipo de Produção`[is.na(sucupira.df.bib$`Subtipo de Produção`)] <- "Indefinido"
sucupira.df.bib$`Categoria do Autor Principal`[is.na(sucupira.df.bib$`Categoria do Autor Principal`)] <- "Indefinido"
```

```{r 2-1-2-bibliografica, eval=all(render_document, has.sucupira.files), echo=FALSE, fig.align='center', fig.pos='H', message=FALSE, warning=FALSE, results='asis'}
# filter sucupira.df.bib by "Tipo de Produção" == "Bibliográfica"
sucupira.df.bib <- unique(sucupira.df.bib[sucupira.df.bib$`Tipo de Produção` == "Bibliográfica", ])
...
colnames(table.bib.long) <- c("Ano", "Produção bibliográfica com discente/egresso vinculada à TCC concluído")
...
if(!sjmisc::is_empty(table.bib)) {
  ...
  table.bib.gt <- gtsummary::tbl_summary(
    ...
    gtsummary::remove_row_type(
      variables = c('Produção bibliográfica com discente/egresso vinculada à TCC concluído'),
      type = "level",
...
```{r 2-1-2-natureza, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
# filter `Item de Detalhamento` = "Natureza" ou "Tipo da Contribuição na Obra"
sucupira.df.nat <- sucupira.df.bib %>%
  dplyr::filter(`Item de Detalhamento` == "Natureza" | `Item de Detalhamento` == "Tipo da Contribuição na Obra"
  )

# patch to replace NA values with "Indefinido"
sucupira.df.nat$`Valor do Item de Detalhamento`[is.na(sucupira.df.nat$`Valor do Item de Detalhamento`)] <- "Indefinido"
...
```{r 2-1-2-local, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
# filter `Item de Detalhamento` = "Idioma"
sucupira.df.local <- sucupira.df.bib %>%
...
# patch to replace NA values with "Indefinido"
sucupira.df.local$`Valor do Item de Detalhamento`[is.na(sucupira.df.local$`Valor do Item de Detalhamento`)] <- "Indefinido"
...
```{r 2-1-2-egressos-data, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
# Discentes
sheet <- "Egressos"
source("Scripts/read-xlsx-sucupira.R", local = knitr::knit_global())
...
```{r 2-1-2-egressos, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
table.egresso <- data.frame(matrix(
  NA,
  nrow = length(unique(sucupira.df.disc$`Identificador do Egresso`)),
  ncol = length(quadrienal.vigente)
))

rownames(table.egresso) <- NULL
table.egresso <- cbind(unique(sucupira.df.disc$`Identificador do Egresso`), table.egresso)
colnames(table.egresso) <- c("ID", as.character(quadrienal.vigente))

# match ID and Ano and fill with Egresso
for(i in 1:dim(table.egresso)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    id <- table.egresso[i, 1]
    ano <- quadrienal.vigente[j]
    table.egresso[i, j + 1] <- ifelse(
...
table.egresso.long <-
  tidyr::gather(dplyr::select(table.egresso, -1),
                "Ano",
                "Titulações",
                colnames(table.egresso)[-1],
                factor_key = TRUE
  )

table.egresso.long <- rbind(
  table.egresso.long,
  data.frame(
    "Ano" = rep(paste0(
      min(quadrienal.vigente), "-", max(quadrienal.vigente)
    ), times = dim(table.egresso)[1]),
    "Titulações" = rep("Titulações", times = dim(table.egresso)[1]),
    check.names = FALSE
  )
)
...
colnames(table.egresso.long) <- c("Ano", "Titulações")

# change all column types to factors
table.egresso.long <- table.egresso.long %>%
  dplyr::mutate(dplyr::across(gtsummary::everything(), as.factor))

if(!sjmisc::is_empty(table.egresso)) {
  # Print tables
  table.egresso.gt <- gtsummary::tbl_summary(
    table.egresso.long,
    by = "Ano",
    statistic = list(gtsummary::all_categorical() ~ "{n}"),
    missing = "no",
  ...
  table.egresso.gt <- list()
}
```

```{r 2-1-2-egressos-nivel, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
...
```{r 2-1-2-qualis, eval = all(render_document, any(has.metricas.doi.ppg, has.metricas.doi.sucupira)), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
...
# get only produção in sucupura.df.bib
metricas_all_ID <- metricas_all_ID[metricas_all_ID$`ID da Produção` %in% sucupira.df.bib$`ID da Produção`,]
...
if(!sjmisc::is_empty(síntese.ano)){
  ...
  table.bib.qualis <- list()
}
```

```{r 2-1-2-razao, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
...
egressos.ano <- table.egresso.long %>%
  na.omit() %>%
  dplyr::filter(Ano != "Ano") %>%
  dplyr::count(Ano)
...
if(!sjmisc::is_empty(producao.ano) && !sjmisc::is_empty(egressos.ano)) {
  # Print tables
...
```{r 2-1-2-list-tables, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
# producão
# List all tables
table.2.1.2.a <- list(
  table.bib.gt,
...
# Drop empty elements from the list
table.2.1.2.a <- table.2.1.2.a[lapply(table.2.1.2.a, length) > 0]
if(any(lapply(table.2.1.2.a, length) > 0)){
  table.2.1.2.a <- gtsummary::tbl_stack(table.2.1.2.a, quiet = TRUE)
} else {
  table.2.1.2.a <- list()
}

# Print HTML table
if (knitr::is_html_output() && !sjmisc::is_empty(table.2.1.2.a)) {
  table.2.1.2.a <- table.2.1.2.a %>%
    gtsummary::modify_source_note("<a href='https://sucupira.capes.gov.br/sucupira/'>Plataforma Sucupira</a>")
...
# Print LaTeX table
if (knitr::is_latex_output() && !sjmisc::is_empty(table.2.1.2.a)) {
  cat('\\FloatBarrier')
  # Convert gtsummary object to kableExtra object
  kable_table <- gtsummary::as_kable_extra(
    x = table.2.1.2.a,
    ...
    booktabs = TRUE,
    linesep = "",
    caption = ifelse(knitr::is_html_output(), "", "Número de trabalhos de discentes/egressos publicados (artigos/livros/capítulos) vinculados a TCC concluído")
  ) %>%
    kableExtra::kable_styling(
      bootstrap_options = c("basic", "hover", "condensed", "responsive"),
      latex_options = c("basic", "repeat_header", "HOLD_position"),
...
# Drop empty elements from the list
table.2.1.2.b <- table.2.1.2.b[lapply(table.2.1.2.b, length) > 0]
if(any(lapply(table.2.1.2.b, length) > 0)){
  table.2.1.2.b <- gtsummary::tbl_stack(table.2.1.2.b, quiet = TRUE)
} else {
  table.2.1.2.b <- list()
}

# Print HTML table
if (knitr::is_html_output() && !sjmisc::is_empty(table.2.1.2.b)) {
  table.2.1.2.b <- table.2.1.2.b %>%
    gtsummary::modify_source_note("<a href='https://sucupira.capes.gov.br/sucupira/'>Plataforma Sucupira</a>")
...
# Print LaTeX table
if (knitr::is_latex_output() && !sjmisc::is_empty(table.2.1.2.b)) {
  cat('\\FloatBarrier')
  # Convert gtsummary object to kableExtra object
  kable_table <- gtsummary::as_kable_extra(
    x = table.2.1.2.b,
...
# Drop empty elements from the list
table.2.1.2.c <- table.2.1.2.c[lapply(table.2.1.2.c, length) > 0]
if(any(lapply(table.2.1.2.c, length) > 0)){
  table.2.1.2.c <- gtsummary::tbl_stack(table.2.1.2.c, quiet = TRUE)
} else {
  table.2.1.2.c <- list()
}

# Print HTML table
if (knitr::is_html_output() && !sjmisc::is_empty(table.2.1.2.c)) {
  table.2.1.2.c <- table.2.1.2.c %>%
    gtsummary::modify_source_note("<a href='https://sucupira.capes.gov.br/sucupira/'>Plataforma Sucupira</a>")
...
# Print LaTeX table
if (knitr::is_latex_output() && !sjmisc::is_empty(table.2.1.2.c)) {
  cat('\\FloatBarrier')
  # Convert gtsummary object to kableExtra object
  kable_table <- gtsummary::as_kable_extra(
    x = table.2.1.2.c,
    ...
    booktabs = TRUE,
    linesep = "",
    caption = ifelse(knitr::is_html_output(), "", "Distribuição de estratos Qualis por ano")
  ) %>%
    kableExtra::kable_styling(
      bootstrap_options = c("basic", "hover", "condensed", "responsive"),
      latex_options = c("basic", "repeat_header", "HOLD_position"),
...
# Drop empty elements from the list
table.2.1.2.d <- table.2.1.2.d[lapply(table.2.1.2.d, length) > 0]
if(any(lapply(table.2.1.2.d, length) > 0)){
  table.2.1.2.d <- gtsummary::tbl_stack(table.2.1.2.d, quiet = TRUE)
} else {
  table.2.1.2.d <- list()
}

# Print HTML table
if (knitr::is_html_output() && !sjmisc::is_empty(table.2.1.2.d)) {
  table.2.1.2.d <- table.2.1.2.d %>%
    gtsummary::modify_source_note("<a href='https://sucupira.capes.gov.br/sucupira/'>Plataforma Sucupira</a>")
...
# Print LaTeX table
if (knitr::is_latex_output() && !sjmisc::is_empty(table.2.1.2.d)) {
  cat('\\FloatBarrier')
  # Convert gtsummary object to kableExtra object
  kable_table <- gtsummary::as_kable_extra(
    x = table.2.1.2.d,
    ...
    booktabs = TRUE,
    linesep = "",
    caption = ifelse(knitr::is_html_output(), "", "Razão (trabalhos de discentes/egressos publicados [artigos/livros/capítulos] por titulação) vinculados a TCC concluído")
  ) %>%
    kableExtra::kable_styling(
      bootstrap_options = c("basic", "hover", "condensed", "responsive"),
      latex_options = c("basic", "repeat_header", "HOLD_position"),
...
valid_ids <- vector()
for (id in unique(sucupira.df.bib$`ID da Produção`)) {
  subset_data <- sucupira.df.bib[sucupira.df.bib$`ID da Produção` == id, ]
...
sucupira.df.bib$`Tipo de Produção` <- tools::toTitleCase(tolower(sucupira.df.bib$`Tipo de Produção`))
sucupira.df.bib$`Subtipo de Produção` <- tools::toTitleCase(tolower(sucupira.df.bib$`Subtipo de Produção`))
sucupira.df.bib$`Nome do Autor` <- tools::toTitleCase(tolower(sucupira.df.bib$`Nome do Autor`))

sucupira.df.bib$`Tipo de Produção`[is.na(sucupira.df.bib$`Tipo de Produção`)] <- "Indefinido"
sucupira.df.bib$`Subtipo de Produção`[is.na(sucupira.df.bib$`Subtipo de Produção`)] <- "Indefinido"

sucupira.df.details$`Categoria do Autor Principal`[is.na(sucupira.df.bib$`Categoria do Autor Principal`)] <- "Indefinido"
```
...
# patch to convert NA to "Indefinido"
sucupira.df.nat$`Valor do Item de Detalhamento`[is.na(sucupira.df.nat$`Valor do Item de Detalhamento`)] <- "Indefinido"
...
sucupira.df.autor <- sucupira.df.bib
...
# check protagonismo docente in sucupira.df.bib$"Número de Ordem do Autor": 1st, 2nd, 2nd last, last
# initialize data
