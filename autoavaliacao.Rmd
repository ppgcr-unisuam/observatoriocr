...
```{r, eval = all(FALSE, render_document, has.sucupira.files), results = "asis"}
glue::glue(knitr::knit(text = out), collapse = '\n\n')
```
...
table.excl$"Nome Docente" <- factor(
  ...
  levels = sort(unique(c(levels(table.excl$"Nome Docente"), docentes.quadrienio)))
)
...
if(!sjmisc::is_empty(table.excl.long)){
  table.excl.gt <- table.excl.long |>
    ...
    gtsummary::modify_header(label = "Docentes permanentes com atuação externa ao PPG", stat_0 = "Total") %>%
    gtsummary::modify_footnote(gtsummary::all_stat_cols() ~ NA) %>%
    gtsummary::bold_labels() 
} else {
  table.excl.gt <- list()
...
if (knitr::is_latex_output() && !sjmisc::is_empty(table.excl.gt)) {
  ...
  kable_table <- gtsummary::as_kable_extra(
    ...
    kableExtra::kable_styling(
      ...
      full_width = TRUE,
      position = "center"
    ) %>% kableExtra::column_spec(column = 1:(1 + (length(unique(table.excl$`Exclusividade`))+1)), width = paste0(c(0.40, rep(0.60/(length(unique(table.excl$`Exclusividade`))+1), times = (length(unique(table.excl$`Exclusividade`))+1)))*(29-3-3), "cm"))
  print(kable_table)
}
...
```{r 2.1.1-detalhe, eval = all(render_document, has.sucupira.files, !sjmisc::is_empty(id.TCC))}
if (!sjmisc::is_empty(id.TCC)) {
  # Create R Markdown chunks dynamically because projetos can be very large tables
