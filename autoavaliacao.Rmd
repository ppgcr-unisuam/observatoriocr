...
`r if(all(!sjmisc::is_empty(out), render_document, has.sucupira.files)){paste(knitr::knit(text = out), collapse = '\n\n')}`
...
if (knitr::is_latex_output() && !sjmisc::is_empty(table.excl.gt)) {
  ...
  kable_table <- gtsummary::as_kable_extra(
    ...
    kableExtra::kable_styling(
      ...
      full_width = TRUE,
      position = "center"
    ) %>% kableExtra::column_spec(column = 1:(1 + (length(unique(table.excl$`Atuação externa`))+1)), width = paste0(c(0.40, rep(0.60/(length(unique(table.excl$`Atuação externa`))+1), times = (length(unique(table.excl$`Atuação externa`))+1)))*(29-3-3), "cm"))
  print(kable_table)
}
