...
```{r avaliacao, eval = all(render_document, has.avaliacao), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
...
# print tables
if(!sjmisc::is_empty(table.pp.financiamento.long)){
  table.pp.auxilio.gt <-
    gtsummary::tbl_summary(
      table.pp.financiamento.long,
      by = "Ano",
...
```{r create-markdown-chunks-dynamically, eval = all(render_document, has.sucupira.files)}
...
if(!sjmisc::is_empty(lista.AC.LP)){
  for (i in 1:length(lista.AC.LP)) {
    ...
    knit_expanded <- paste0(
      "\n```{r, eval = all(!sjmisc::is_empty(lista.AC.LP), render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}\n",
      "cat('\\n\\n<!-- -->\\n\\n')\n",
      "# print tables\n",
      "if (knitr::is_html_output()) {\n",
      "  # use kable for HTML output\n",
...
`r if(all(!sjmisc::is_empty(lista.AC.LP), render_document, has.sucupira.files)){paste(knitr::knit(text = out), collapse = '\n\n')}`
...
if(!sjmisc::is_empty(coerencia)){
  # print tables tbl_cross with thee levels
  coerencia.gt <- coerencia |>
    gtsummary::tbl_strata(
      strata = 'Área de Concentração',
      .tbl_fun =
        ~ .x |>
        gtsummary::tbl_summary(
          by = 'Linha de Pesquisa',
          statistic = list(
        ...
        gtsummary::modify_header(label = "**Coerência e Abrangência**") %>%
        gtsummary::modify_footnote(gtsummary::all_stat_cols() ~ NA) %>%
        gtsummary::bold_labels(),
...
# remove 0s from gtsummary
# coerencia.gt[1]$table_body <- coerencia.gt[1]$table_body %>% 
#   dplyr::mutate(dplyr::across(sprintf("stat_%d_1", 1:nlevels(coerencia$`Linha de Pesquisa`)), ~gsub("^0.*", "",.)))

# Print HTML table
if (knitr::is_html_output() && !sjmisc::is_empty(coerencia)) {
  print(coerencia.gt)
}

# Print LaTeX table
if (knitr::is_latex_output() && !sjmisc::is_empty(coerencia)) {
  cat('\\FloatBarrier')
  # Convert gtsummary object to kableExtra object
  kable_table <- gtsummary::as_kable_extra(
    x = coerencia.gt,
    ...
    booktabs = TRUE,
    linesep = "",
    caption = ifelse(knitr::is_html_output(), "", "Coerência e abrangência conceitual - Área de concentracão -> Linha de Pesquisa -> Projeto de Pesquisa"),
  ) %>%
    kableExtra::kable_styling(
      bootstrap_options = c("basic", "hover", "condensed", "responsive"),
      latex_options = c("basic", "repeat_header", "HOLD_position"),
      ...
      full_width = TRUE,
      position = "center"
    ) %>% kableExtra::column_spec(column = 1:(1 + nlevels(coerencia$`Área de Concentração`)*nlevels(coerencia$`Linha de Pesquisa`)), width = paste0(c(0.30, rep(0.70/(nlevels(coerencia$`Área de Concentração`)*nlevels(coerencia$`Linha de Pesquisa`)), times = nlevels(coerencia$`Área de Concentração`)*nlevels(coerencia$`Linha de Pesquisa`)))*(29-3-3), "cm"))
  print(kable_table)
}
...
sucupira.df.ext <- data.frame(do.call(dplyr::bind_rows, sucupira))
...
# as numeric
table.excl[, 2:9] <- lapply(table.excl[, 2:9], as.numeric)

# sum columns by Nome docente
table.excl <- table.excl %>% 
  dplyr::mutate(
    `Exclusividade` = rowSums(dplyr::select(., -`Nome Docente`))
  )
...
table.excl$"Nome Docente" <- factor(
  ...
  levels = sort(unique(c("Docentes", levels(table.excl$"Nome Docente"), docentes.quadrienio)))
)
...
if(!sjmisc::is_empty(table.excl.long)){
  table.excl.gt <- table.excl.long |>
    ...
    gtsummary::modify_header(label = "Exclusividade no PPG", stat_0 = "Total") %>%
    gtsummary::modify_footnote(gtsummary::all_stat_cols() ~ NA) %>%
    gtsummary::bold_labels() 
} else {
  table.excl.gt <- list()
...
if (knitr::is_latex_output() && !sjmisc::is_empty(table.excl.gt)) {
  ...
  kable_table <- gtsummary::as_kable_extra(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = ifelse(knitr::is_html_output(), "", "Exclusividade no PPG - Total"),
  ) %>%
    kableExtra::kable_styling(
      bootstrap_options = c("basic", "hover", "condensed", "responsive"),
      latex_options = c("basic", "repeat_header", "HOLD_position"),
      ...
      full_width = TRUE,
      position = "center"
    ) %>% kableExtra::column_spec(column = 1:(1 + (length(unique(table.excl.gt$`Exclusividade`))+1)), width = paste0(c(0.40, rep(0.60/(length(unique(table.excl.gt$`Exclusividade`))+1), times = (length(unique(table.excl.gt$`Exclusividade`))+1)))*(29-3-3), "cm"))
  print(kable_table)
}
...
```{r planejamento, eval = all(render_document, has.planejamento, knitr::is_html_output()), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
# Initialize all sheets
...
sucupira.perfil.disc <- data.frame(do.call(dplyr::bind_rows, sucupira))
...
if (!sjmisc::is_empty(id.TCC)) {
  ...
  # Print HTML table
  if (knitr::is_html_output() && !sjmisc::is_empty(table.2.1.1)) {
    print(table.2.1.1)
  }
  
  # Print LaTeX table
  if (knitr::is_latex_output() && !sjmisc::is_empty(table.2.1.1)) {
    cat('\\FloatBarrier')
    # Convert gtsummary object to kableExtra object
    kable_table <- gtsummary::as_kable_extra(
      x = table.2.1.1,
...
```{r 2.1.1-detalhe, eval = all(!sjmisc::is_empty(id.TCC), render_document, has.sucupira.files)}
...
if (knitr::is_html_output()) {
  ...
  knitr::kable(
    ...
    full_width = TRUE,
    position = "center"
  ) %>% kableExtra::column_spec(column = 1:length(unique(table.projetos.doc$`Linha de Pesquisa`)), width = paste0(c(0.05, 0.20, rep(0.65/length(unique(table.projetos.doc$`Linha de Pesquisa`)), times = length(unique(table.projetos.doc$`Linha de Pesquisa`))), 0.10)*(21-3-3), "cm")) %>%
      kableExtra::column_spec(column = 1, bold = TRUE) %>%
      kableExtra::row_spec(row = 0, bold = TRUE)
} else {
  # use tinytable for LaTeX output
  ...
  tinytable::tt(
    table.doc,
    width = c(0.05, 0.20, rep(0.65/length(unique(table.projetos.doc$`Linha de Pesquisa`)), times = length(unique(table.projetos.doc$`Linha de Pesquisa`))), 0.10),
    style = "bootstrap",
    align = "l",
...
sucupira.df.grad <- data.frame(do.call(dplyr::bind_rows, sucupira))
...
if (knitr::is_latex_output() && !sjmisc::is_empty(table.2.5.5a)) {
  ...
  kable_table <- gtsummary::as_kable_extra(
    ...
    kableExtra::kable_styling(
      ...
      full_width = TRUE,
      position = "center"
    ) %>% kableExtra::column_spec(column = 1:(1 + (length(unique(sucupira.df.grad$Anos))+1)*length(spann)), width = paste0(c(0.40, rep(0.60/((length(unique(sucupira.df.grad$Anos))+1)*length(spann)*length(spann)), times = (length(unique(sucupira.df.grad$Anos))+1)*length(spann)))*(29-3-3), "cm"))
  print(kable_table)
}
...
# drop year column
table.insercao <- table.insercao %>% dplyr::select(
  "Nome Docente",
  "Orientações de Monografia",
  "Orientações de Iniciação Científica",
  "Orientações de Tutoria",
...
# as numeric
table.insercao[, 2:6] <- lapply(table.insercao[, 2:6], as.numeric)

# sum columns by Nome docente
table.insercao <- table.insercao %>% 
  dplyr::mutate(
    `Inserção na Graduação` = rowSums(dplyr::select(., -`Nome Docente`))
  )

# drop rows with 0 values
table.insercao <- table.insercao %>%
  dplyr::filter(
    `Inserção na Graduação` > 0
  )

# drop Inserção na Graduação
table.insercao <- table.insercao %>%
  dplyr::select(
    -"Inserção na Graduação"
)
...
table.insercao <- table.insercao %>%
  tidyr::pivot_longer(
    cols = -c("Nome Docente"),
    names_to = "Inserção na Graduação"
  )
...
table.insercao$"Nome Docente" <- factor(
  ...
  levels = sort(unique(c("Docentes", levels(table.insercao$"Nome Docente"), docentes.quadrienio)))
)
...
if(!sjmisc::is_empty(table.insercao.long)){
  table.insercao.gt <- table.insercao.long |>
    gtsummary::tbl_cross(
      row = 'Nome Docente',
      col = "Inserção na Graduação",
      statistic = "{n}",
      missing = "no",
      digits = 0,
      margin = "column"
    ) %>%
    gtsummary::modify_header(label = "**Disciplinas na Graduação - Inserção**", stat_0 = "Total") %>%
    gtsummary::modify_footnote(gtsummary::all_stat_cols() ~ NA) %>%
    gtsummary::bold_labels() 
} else {
...
if (knitr::is_latex_output() && !sjmisc::is_empty(table.insercao.gt)) {
  ...
  kable_table <- gtsummary::as_kable_extra(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = ifelse(knitr::is_html_output(), "", "Inserção na Graduação - Total"),
  ) %>%
    kableExtra::kable_styling(
      bootstrap_options = c("basic", "hover", "condensed", "responsive"),
      latex_options = c("basic", "repeat_header", "HOLD_position"),
      ...
      full_width = TRUE,
      position = "center"
    ) %>% kableExtra::column_spec(column = 1:(1 + (length(unique(table.insercao$`Inserção na Graduação`))+1)), width = paste0(c(0.40, rep(0.60/(length(unique(table.insercao$`Inserção na Graduação`))+1), times = (length(unique(table.insercao$`Inserção na Graduação`))+1)))*(29-3-3), "cm"))
  print(kable_table)
}
