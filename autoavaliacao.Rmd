...
output:
  ...
  bookdown::pdf_book:
    ...
    includes:
      ...
      before_body: tex/cover_1.tex
    latex_engine: xelatex
    keep_tex: no
---

```{r cover-one, fig.align = 'right', echo = FALSE, include = identical(knitr:::pandoc_to(), 'html'), fig.link = 'images/Cover_1.png', out.width = "100%", eval = !knitr::is_html_output()}
...
```{r, eval = knitr::is_html_output()}
cat("### **Síntese dos dados** {#sintese-dados .tabset}")
```

```{r, eval = !knitr::is_html_output()}
cat("### **Síntese dos dados** {-}")
```
...
source("Scripts/fonte_capes.R", local = knitr::knit_global())
```
...
for (i in 1:length(abas)) {
  ...
  if (dim(periodo.raw)[1] != 0) {
    source("Scripts/table-with-buttons.R", local = knitr::knit_global())
    cat(knitr::knit_print(create_dt(periodo.raw, title = "Documentos CAPES")))
  } else {
    cat("*Sem dados para exibir*")
  }
...
options(knitr.kable.NA = '')
print(
  kable(table.posdoc, align = c("l", rep(
