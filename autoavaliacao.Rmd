...
colnames(table.disc) <- c("ID da Produção", as.character(quadrienal.vigente))
...
colnames(table.nivel) <- c("ID da Produção", as.character(quadrienal.vigente))
...
for(i in 1:dim(table.nivel)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.nivel[i, j + 1] <- ifelse(
      id %in% sucupira.df.disc$`Identificador do Discente`[sucupira.df.disc$Anos == ano],
      sucupira.df.disc$`Nível Discente`[match(sucupira.df.disc$`Identificador do Discente`[i], sucupira.df.disc$`Identificador do Discente`)],
      NA
    )
  }
}
