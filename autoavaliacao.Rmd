...
for(i in 1:dim(table.bib)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.bib[i, j + 1] <- ifelse(
      ...
      sucupira.df.bib$`Subtipo de Produção`[match(sucupira.df.bib$`ID da Produção`[i], sucupira.df.bib$`ID da Produção`)],
      NA
    )
  }
}
...
```{r 2-2-1-autor-principal, eval = has.sucupira.files, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
table.aut <- data.frame(matrix(
  NA,
  nrow = length(unique(sucupira.df.anais$`ID da Produção`)),
  ncol = length(quadrienal.vigente)
))

rownames(table.aut) <- NULL
table.aut <- cbind(unique(sucupira.df.anais$`ID da Produção`), table.aut)
...
for(i in 1:dim(table.aut)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.aut[i, j + 1] <- ifelse(
      id %in% sucupira.df.anais$`ID da Produção`[sucupira.df.anais$Anos == ano],
      sucupira.df.anais$`Categoria do Autor Principal`[match(sucupira.df.anais$`ID da Produção`[i], sucupira.df.anais$`ID da Produção`)],
      NA
    )
  }
}
...
for(i in 1:dim(table.nat)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.nat[i, j + 1] <- ifelse(
      ...
      sucupira.df.nat$`Valor do Item de Detalhamento`[match(sucupira.df.nat$`ID da Produção`[i], sucupira.df.nat$`ID da Produção`)],
      NA
    )
  }
}
...
colnames(table.nat.long) <- c("Ano", "Natureza")
...
for(i in 1:dim(table.local)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.local[i, j + 1] <- ifelse(
      ...
      sucupira.df.local$`Valor do Item de Detalhamento`[match(sucupira.df.local$`ID da Produção`[i], sucupira.df.local$`ID da Produção`)],
      NA
    )
  }
}
...
if (knitr::is_latex_output()) {
  ...
  kable_table <- gtsummary::as_kable_extra(
    ...
    booktabs = TRUE,
    linesep = "",
    caption = ifelse(knitr::is_html_output(), "", "Produção do corpo discente em eventos científicos")
  ) %>%
    kableExtra::kable_styling(
      bootstrap_options = c("basic", "hover", "condensed", "responsive"),
      latex_options = c("basic", "repeat_header", "HOLD_position"),
...
for(i in 1:dim(table.prd)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.prd[i, j + 1] <- ifelse(
      id %in% sucupira.df$`ID da Produção`[sucupira.df$Anos == ano],
      sucupira.df$`Tipo de Produção`[match(table.prd$`ID da Produção`[i], sucupira.df$`ID da Produção`)],
      NA
    )
  }
}
...
for(i in 1:dim(table.tec)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.tec[i, j + 1] <- ifelse(
      ...
      sucupira.df.tec$`Subtipo de Produção`[match(sucupira.df.tec$`ID da Produção`[i], sucupira.df.tec$`ID da Produção`)],
      NA
    )
  }
}
...
for(i in 1:dim(table.tcc)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.tcc[i, j + 1] <- ifelse(
      id %in% sucupira.df$`ID da Produção`[sucupira.df$Anos == ano],
      sucupira.df$`Produção vinculada a TCC concluído`[match(sucupira.df$`ID da Produção`[i], sucupira.df$`ID da Produção`)],
      NA
    )
  }
}
...
for(i in 1:dim(table.ac)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.ac[i, j + 1] <- ifelse(
      id %in% sucupira.df$`ID da Produção`[sucupira.df$Anos == ano],
      sucupira.df$`Área de Concentração`[match(sucupira.df$`ID da Produção`[i], sucupira.df$`ID da Produção`)],
      NA
    )
  }
}
...
for(i in 1:dim(table.lp)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.lp[i, j + 1] <- ifelse(
      id %in% sucupira.df$`ID da Produção`[sucupira.df$Anos == ano],
      sucupira.df$`Linha de Pesquisa`[match(sucupira.df$`ID da Produção`[i], sucupira.df$`ID da Produção`)],
      NA
    )
  }
}
...
for(i in 1:dim(table.aut)[1]) {
  for(j in 1:length(quadrienal.vigente)) {
    ...
    table.aut[i, j + 1] <- ifelse(
      id %in% sucupira.df$`ID da Produção`[sucupira.df$Anos == ano],
      sucupira.df$`Categoria do Autor Principal`[match(sucupira.df$`ID da Produção`[i], sucupira.df$`ID da Produção`)],
      NA
    )
  }
}
