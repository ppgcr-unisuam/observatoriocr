...
`r if(all(is.null("out.1"), render_document, has.sucupira.files)){paste(knitr::knit(text = out.1), collapse = '\n\n')}`
...
if (knitr::is_latex_output() && !sjmisc::is_empty(table.excl.gt)) {
  ...
  kable_table <- gtsummary::as_kable_extra(
    ...
    kableExtra::kable_styling(
      ...
      full_width = TRUE,
      position = "center"
    ) %>% kableExtra::column_spec(column = 1:(1 + (nlevels(factor(table.excl$`Atuação exclusiva`))+nlevels(factor(table.excl$table.excl.long)))), width = paste0(c(0.40, rep(0.60/((nlevels(factor(table.excl$`Atuação exclusiva`))+nlevels(factor(table.excl$table.excl.long)))), times = ((nlevels(factor(table.excl$`Atuação exclusiva`))+nlevels(factor(table.excl$table.excl.long))))))*(29-3-3), "cm"))
  print(kable_table)
}
...
<!--- knit those table chunk statements --> 
`r if(all(is.null("out.2"), render_document, has.sucupira.files)){paste(knitr::knit(text = out.2), collapse = '\n\n')}`
