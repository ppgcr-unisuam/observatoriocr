...
if (knitr::is_latex_output() && !sjmisc::is_empty(table.2.5.5a)) {
  ...
  kable_table <- gtsummary::as_kable_extra(
    ...
    kableExtra::kable_styling(
      ...
      full_width = TRUE,
      position = "center"
    ) %>% kableExtra::column_spec(column = 1:(1 + (length(unique(sucupira.df.grad$Anos))+1)*length(spann)), width = paste0(c(0.55, rep(0.45/((length(unique(sucupira.df.grad$Anos))+1)*length(spann)*length(spann)), times = (length(unique(sucupira.df.grad$Anos))+1)*length(spann)))*(29-3-3), "cm"))
  print(kable_table)
}
