...
if (nlevels(sucupira.perfil.disc$`Nível Discente`) != 0) {
...
```{r 2.1.1-detalhe, eval = all(render_document, has.sucupira.files)}
if (!sjmisc::is_empty(table.bancas.long)) {
  # Create R Markdown chunks dynamically because projetos can be very large tables
...
```{r 2-2-1-bibliografica, eval = all(render_document, has.sucupira.files), echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
...
# Drop empty elements from the list
table.2.5.5 <- table.2.5.5[lapply(table.2.5.5, length) > 0]

spann <- c("**Orientações de Monografia**", "**Orientações de Iniciação Científica**", "**Orientações de Tutoria**")
spann <- spann[which(lapply(table.2.5.5, length) > 0)]

if(any(lapply(table.2.5.5, length) > 0)){
  table.2.5.5 <- gtsummary::tbl_merge(
    tbls = table.2.5.5,
    tab_spanner = spann
  )
} else {
...
# Print HTML table
if (knitr::is_html_output() && !sjmisc::is_empty(table.2.5.5)) {
  print(table.2.5.5)
}

# Print LaTeX table
if (knitr::is_latex_output() && !sjmisc::is_empty(table.2.5.5)) {
  cat('\\FloatBarrier')
  # Convert gtsummary object to kableExtra object
  kable_table <- gtsummary::as_kable_extra(
    x = table.2.5.5,
    ...
    kableExtra::kable_styling(
      ...
      full_width = TRUE,
      position = "center"
    ) %>% kableExtra::column_spec(column = 1:(1 + length(unique(sucupira.df.grad$Anos))*nlevels(sucupira.df.grad$"Nível Discente")), width = paste0(c(0.55, rep(0.45/(length(unique(sucupira.df.grad$Anos))*length(spann)), times = length(unique(sucupira.df.grad$Anos))*length(spann)))*(29-3-3), "cm"))
  print(kable_table)
}
...
# Print HTML table
if (knitr::is_html_output() && !sjmisc::is_empty(table.3.2.1)) {
  print(table.3.2.1)
}

# Print LaTeX table
if (knitr::is_latex_output() && !sjmisc::is_empty(table.3.2.1)) {
  cat('\\FloatBarrier')
  # Convert gtsummary object to kableExtra object
  kable_table <- gtsummary::as_kable_extra(
    x = table.3.2.1,
