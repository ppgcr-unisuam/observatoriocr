...
=======
if (knitr::is_html_output()) {
>>>>>>> c61acea8dcee61989a38c20931b8c41f8be7ecb6
  print(table.1.2.5)
}
...
=======
    ) %>% kableExtra::column_spec(column = 1:(1 + length(unique(turmas$"Período/Ano"))), width = paste0(c(0.15, rep(0.85/(length(unique(turmas$"Período/Ano"))), times = length(unique(turmas$"Período/Ano"))))*(21-3-3), "cm"))
>>>>>>> c61acea8dcee61989a38c20931b8c41f8be7ecb6
  print(kable_table)
}
...
```{r 2-5-3-data, eval = has.sucupira.files, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
# Produções - Autores
...
```{r 2-5-3-orient-matr, eval = has.sucupira.files, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
...
table.princ <- data.frame(matrix(
  NA,
  nrow = length(unique(sucupira.df.princ$`Identificador do Orientador`)),
  ncol = length(quadrienal.vigente)
))

table.princ <- table(sucupira.df.princ$`Identificador do Orientador`, sucupira.df.princ$Anos)

# match Identificador do Orientador to Nome do Orientador
for(i in 1:dim(table.princ)[1]) {
  rownames(table.princ)[i] <- sucupira.df.princ$`Nome do Orientador`[which(sucupira.df.princ$`Identificador do Orientador` == rownames(table.princ)[i])]
}

# convert to data.frame
table.princ <- as.data.frame(table.princ)
# rename columns
colnames(table.princ) <- c("Nome do Orientador", "Ano", "Orientandos")

# replicate each Nome do Orientador by the value of Orientandos
table.princ.long <- data.frame(
  "Ano" = rep(table.princ$Ano, times = table.princ$Orientandos),
  "Nome do Orientador" = rep(table.princ$`Nome do Orientador`, times = table.princ$Orientandos),
  "Orientandos" = rep(table.princ$Orientandos, times = table.princ$Orientandos),
  check.names = FALSE
)

# drop Orientandos column
table.princ.long <- table.princ.long[, -3]

table.princ.long <- rbind(
  table.princ.long,
  data.frame(
    "Ano" = rep(paste0(
      min(quadrienal.vigente), "-", max(quadrienal.vigente)
    ), times =  length(unique(sucupira.df.princ$`Identificador do Discente`))),
    "Nome do Orientador" = rep("Orientador", times = length(unique(sucupira.df.princ$`Identificador do Discente`))),
    check.names = FALSE
  )
)

my_N <- function(data, ...) {
  gtsummary::style_number(sum(data == "Orientador", na.rm = TRUE), digits = 0)
}

colnames(table.princ.long) <- c("Ano", "Nome Docente")

# change all column types to factors
table.princ.long <- table.princ.long %>%
  dplyr::mutate(dplyr::across(gtsummary::everything(), as.factor))

# check for missing docentes.quadrienio
table.princ.long$"Nome Docente" <- factor(
  table.princ.long$"Nome Docente",
  levels = sort(unique(c("Docentes", levels(table.princ.long$"Nome Docente"), docentes.quadrienio)))
)

# Print tables
table.princ.gt <- gtsummary::tbl_summary(
  table.princ.long,
  by = "Ano",
  statistic = list(gtsummary::all_categorical() ~ "{n}"),
  missing = "no",
  ...
  gtsummary::remove_row_type(
    variables = c('Nome Docente'),
    type = "level",
    level_value = ("Orientador")
  ) %>%
  gtsummary::modify_column_hide(columns = paste0("stat_", as.character(length(quadrienal.vigente) + 1)))

# Print HTML table
if (knitr::is_html_output()) {
  print(table.princ.gt)
}
...
```{r 2-5-4-orient-tit, eval = has.sucupira.files, echo = FALSE, warning = FALSE, message = FALSE, fig.align = 'center', results = 'asis', fig.pos='H'}
<<<<<<< HEAD
# filter sucupira.df.orient by "Orientador Principal?" == "Sim"
...
table.princ <- as.data.frame(table.princ)
# rename columns
colnames(table.princ) <- c("Nome do Orientador", "Ano", "Titulados")

# replicate each Nome do Orientador by the value of Titulados
table.princ.long <- data.frame(
  "Ano" = rep(table.princ$Ano, times = table.princ$Titulados),
  "Nome do Orientador" = rep(table.princ$`Nome do Orientador`, times = table.princ$Titulados),
  "Titulados" = rep(table.princ$Titulados, times = table.princ$Titulados),
  check.names = FALSE
)

# drop Titulados column
table.princ.long <- table.princ.long[, -3]
