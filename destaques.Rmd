...
abas <- readxl::excel_sheets("PPG/Destaques.xlsx")
...
for (j in 1:length(abas)) {
  ...
  destaques.raw <- readxl::read_excel("PPG/Destaques.xlsx",
                                      sheet = abas[j],
                                      col_types = c("text"))
  
  # Replace the link by a tag
  Abrir <- matrix(NA, nrow = dim(destaques.raw)[1])
  colnames(Abrir) <- "Abrir"
  
  # Add hyperlinks
  for (i in 1:dim(destaques.raw)[1]) {
    if (!is.na(destaques.raw[i, 3])) {
      Abrir[i] <- paste0('<a href=\"', destaques.raw[i, 3], '\" target=\"_blank\">', '↗️' ,'</a>')
    }
  }
  
  destaques <- cbind(destaques.raw[, -3], Abrir)
  
  # Print table or message if no data
  if (dim(destaques)[1] != 0) {
    source("Scripts/table-with-buttons.R", local = knitr::knit_global())
    cat(knitr::knit_print(create_dt(destaques, title = "Destaques - Egressos")))
  } else {
...
for (id in 1:length(orcids)) {
  ...
  cat('##### **', editor.name, '**', sep = "")
  cat('\n\n<!-- -->\n\n')
  knitr::knit_print(source("Scripts/invited-positions.R", local = knitr::knit_global()))
  cat('\n\n<!-- -->\n\n')
  cat('<br><a style="float:right" href="#top"><b>Início &nbsp;</b>⬆️</a><br>')
}
```
...
for (id in 1:length(orcids)) {
  ...
  cat('\n\n<!-- -->\n\n')
  knitr::knit_print(source("Scripts/reviewer-board.R", local = knitr::knit_global()))
  cat('\n\n<!-- -->\n\n')
  cat(
...
for (j in 1:length(abas)) {
  ...
  for (i in 1:dim(news)[1]) {
    news[i, 1] <- paste("#### **", as.Date(as.numeric(news[i, 1]), origin = "1899-12-30"), "**", sep = "")
    news[i, 3] <- paste0('<a href="', news[i,3],'" target="_blank">', "↗️" ,"</a>")
  }
...
for (j in 1:length(abas)) {
  ...
  cat('\n\n<!-- -->\n\n')
  print(
    knitr::kable(news.list[[j]], align = c("l", rep("c", ncol(news) - 2), "r"), escape = FALSE) %>% kableExtra::kable_styling(
