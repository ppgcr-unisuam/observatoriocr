...
for (j in 1:length(abas)) {
  ...
  ids <- order(destaques[, 1], decreasing = TRUE)
  destaques <- destaques[ids,]

  # print table
  if (dim(destaques)[1] != 0) {
    source("Scripts/table-with-buttons.R", local = knitr::knit_global())
    knit_print(create_dt(destaques, title = "Destaques - Egressos"))
  } else {
...
for (j in 1:length(abas)) {
  ...
  ids <- order(premios[, 1], decreasing = TRUE)
  premios <- premios[ids,]

  # print table
  if (dim(premios)[1] != 0) {
    source("Scripts/table-with-buttons.R", local = knitr::knit_global())
    knit_print(create_dt(premios, title = "Destaques - Prêmios"))
  } else {
    cat("*Sem dados para exibir*")
  }
