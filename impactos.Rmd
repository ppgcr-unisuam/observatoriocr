...
# stack tables
table.2.5.1 <- gtsummary::tbl_stack(
  list(table.part1, table.part2)
)
...
```{r pandemia-filtro-resumos, eval = any(has.editoria.ppg, has.editoria.sucupira)}
# dplyr::selecting those with COVID in the title
try(doi_with_altmetric_editoria_impacto <-
      doi_with_altmetric_editoria[grep(paste(tolower(pandemia), collapse = "|"),
                              tolower(doi_with_altmetric_editoria$title)),], silent = TRUE)
try(doi_without_altmetric_editoria_impacto <-
      doi_without_altmetric_editoria[grep(paste(tolower(pandemia), collapse = "|"),
                                 tolower(doi_without_altmetric_editoria$title)),],
    silent = TRUE)
try(references_editoria_impacto <-
      references_editoria[grep(paste(tolower(pandemia), collapse = "|"), tolower(references_editoria)[, 2]),]
    , silent = TRUE
)
```
...
# dplyr::selecting those with COVID in the title
financiadores <- financiadores[grep(paste(tolower(pandemia), collapse = "|"),
                     tolower(financiadores$`Título do projeto`)),]
fonte <- fonte[grep(paste(tolower(pandemia), collapse = "|"),
             tolower(financiadores$`Título do projeto`)),]
...
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(financiadores, title = "Impactos - Ações durante a Pandemia - Financiadores")

cat('\n\n')
cat("**Fontes**: [**CAPES**](https://www.gov.br/capes/pt-br), [**CNPq**](https://www.gov.br/cnpq/pt-br), [**FAPERJ**](http://www.faperj.br), **UNISUAM**")
cat('\n\n')
cat('<br><a style="float:right" href="#top"><b>Início &nbsp;</b>', fontawesome::fa("circle-arrow-up"), '</a><br>')
```
...
if (dim(tcc)[1] != 0) {
  ...
  table.tcc <- table.tcc %>% dplyr::arrange(dplyr::desc(table.tcc$'Ano'), table.tcc$'Autor')
  
  table.tcc <- table.tcc %>% dplyr::arrange(dplyr::desc(as.numeric(table.tcc$'Ano')), table.tcc$'Título')
}
# print table
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
...
```{r pandemia-artigos-periodicos-icons, eval = any(has.artigos.completos.ppg, has.artigos.completos.sucupira)}
# bind sources
try(doi_with_altmetric <- dplyr::bind_rows(doi_with_altmetric_artigos_impacto,
                                           doi_with_altmetric_editoria_impacto),
    silent = TRUE)
try(doi_without_altmetric <- dplyr::bind_rows(doi_without_altmetric_artigos_impacto,
                                              doi_without_altmetric_editoria_impacto),
    silent = TRUE)
try(references <- dplyr::bind_rows(references_artigos_impacto, references_editoria_impacto),
    silent = TRUE)
...
```{r, eval = !any(has.livros.ppg)}
cat('<br>')
```

```{r pandemia-livros-icons, eval = has.livros.ppg}
# bind sources
...
```{r, eval = !any(has.anais.ppg)}
cat('<br>')
```

```{r pandemia-trabalhos-anais-icons, eval = has.anais.ppg}
# bind sources
...
### **Apresentação de trabalho** {.tabset}
