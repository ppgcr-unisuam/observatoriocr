...
### **Disciplinas** {#turmas-disciplinas}
...
### **Docentes e Pós-Docs** {#docentes-pos-docs}
...
names(sucupira.df) <- names(sucupira[[as.character(max(as.numeric(periodo.vigente)))]])
...
colnames(table.prd) <- c("ID da Produção", as.character(periodo.vigente))
...
colnames(table.bib) <- c("ID da Produção", as.character(periodo.vigente))
...
colnames(table.tec) <- c("ID da Produção", as.character(periodo.vigente))
...
colnames(table.tcc) <- c("ID da Produção", as.character(periodo.vigente))
...
colnames(table.ac) <- c("ID da Produção", as.character(periodo.vigente))
...
colnames(table.lp) <- c("ID da Produção", as.character(periodo.vigente))
...
colnames(table.aut) <- c("ID da Produção", as.character(periodo.vigente))
...
cat('\n\n')
cat('**Nota**: A classificação dos periódicos pelo Qualis refere-se à Avaliação Quadrienal 2017-2020 conforme arquivo disponibilizado na [Plataforma Sucupira](https://sucupira.capes.gov.brpublic/consultas/coleta/veiculoPublicacaoQualis/listaConsultaGeralPeriodicos.xhtml) e não substitui a classificação da CAPES da Avaliação Quadrienal ', min(periodo.vigente), '-', max(periodo.vigente), '.', sep = "")
cat('\n\n')
```
...
```{r periodicos-ies, eval = has.periodicos}
...
source("Scripts/fonte-agencias.R", local = knitr::knit_global())
```
...
```{r politicas-citacao, eval = all(has.sage.files, curl::has_internet())}
...
# Bind all sheets for disciplines
periodos <- names(sucupira.list)[na.omit(match(periodo.vigente, names(sucupira.list)))]
projetos.all <- data.frame(do.call(dplyr::bind_rows, sucupira.list[periodos]), check.names = FALSE)
...
tcc.all <- data.frame(do.call(dplyr::bind_rows, sucupira.list[periodos]), check.names = FALSE)
...
# print table
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(table.tese, title = "Impacto - Teses defendidas durante a Pandemia")
...
# print table
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(table.tese, title = "Impacto - Dissertações defendidas durante a Pandemia")
