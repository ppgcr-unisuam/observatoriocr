...
cat('^[Regiões correspondem à quantidade de linhas de pesquisa.]')
cat('\n')
cat('^[Áreas internas correspondem à quantidade de projetos na respectiva linha de pesquisa.]'
)
```
...
cat('\n\n')
cat("*Fontes:* [**Plataforma Sucupira**](https://sucupira.capes.gov.br/sucupira/)")
cat('^[Termos nas nuvens correspondem àqueles encontrados nos títulos dos projetos de pesquisa, trabalho de conclusão de curso e produç˜ão intelectual, respectivamente.]')
```
...
cat('\n\n')
cat("*Fontes:* [**Plataforma Sucupira**](https://sucupira.capes.gov.br/sucupira/)")
cat('^[Quantidades referem-se ao número de projetos de pesquisa ativos no ano.]')
```
...
artigos <-
  rbind(
    dplyr::select(artigos,
           "Publicado.em",
           "DOI",
           "ISSN"),
    dplyr::select(resumos,
           "Publicado.em",
           "DOI",
           "ISSN"),
    dplyr::select(eventos,
           "Publicado.em",
           "DOI",
           "ISSN")
  )

# remove os artigos sem DOI
artigos <- artigos[complete.cases(artigos$DOI), ]

# remove os aceitos, se existentes
artigos <-
  artigos[artigos$Publicado.em != "Aceito",]

# adding Webqualis column data
artigos$WebQualis <-
  rep("Sem Qualis", times  = length(dim(artigos)[1]))
for (i in 1:dim(artigos)[1]) {
  try(artigos$WebQualis[i] <-
        qualis$Estrato[match(artigos$ISSN[i], qualis$ISSN)])
}
# adding Journal Name column data
artigos$Título <-
  rep("Sem Qualis", times  = length(dim(artigos)[1]))
for (i in 1:dim(artigos)[1]) {
  try(artigos$Título[i] <-
        qualis$Título[match(artigos$ISSN[i], qualis$ISSN)])
}
...
for (i in 1:length(estratos)) {
  qualis.PPG[i] <-
    sum(artigos$WebQualis == estratos[i], na.rm = TRUE) / length(artigos$WebQualis) * 100
}
...
for (i in 1:dim(artigos)[1]) {
  c.score.i <-
    as.numeric(citescore$x2021_cite_score[grep(gsub("-", "", artigos$ISSN[i]), citescore$issn)])
  c.score <-
    c(c.score, ifelse(identical(c.score.i, numeric(0)) |
                    all(is.na(c.score.i)), NA, c.score.i))
}
...
table.3.median <-
  aggregate(
    table.3,
    by = list(table.3$artigos.Publicado.em),
    FUN = median,
    na.rm = TRUE
  )
...
table.4 <- data.frame(artigos$Publicado.em, c.score)
table.4.median <-
  aggregate(
    table.4,
    by = list(table.4$artigos.Publicado.em),
    FUN = median,
    na.rm = TRUE
  )
...
vioplot(as.numeric(table.4[, 2]) ~ as.numeric(table.4[, 1]),
        xlab = NA,
        ylab = "CiteScore")
...
data.journals <-
  na.omit(data.frame(name = journals.SJR$artigos.Título[order(journals.SJR$SJR, decreasing = TRUE)][1:top],
                     value = as.numeric(
                       sort(journals.SJR$SJR, decreasing = TRUE)[1:top]
                     )))
...
CITESCORE.base <- na.omit(citescore$x2021_cite_score)
CITESCORE.PPG <- na.omit(c.score)
...
cat('^[Produções com DOI apenas; a produção pode estar subsestimada pois produtos sem DOI não estão apresentados.]')
```
...
```{r sintese-visibilidade, include = FALSE, eval = has.producao}
# get years
source("Scripts/years-sucupira.R", local = knitr::knit_global())
...
doi_unique <-
  bind_rows(doi_unique_artigos,
            ...
            doi_unique_eventos)
table.1 <- 
  table.1 <- data.frame(doi_unique$published_on, doi_unique$score)
...
vioplot(as.numeric(table.1[, 2]) ~ as.numeric(table.1[, 1]),
        xlab = NA,
        ylab = "Altmetric")
...
par(new = TRUE)
my_image <- readPNG("Metrics/Altmetric/cropped-altmetric-symbol.png")
...
text(
  x = (x1 + x2) / 2,
  y = (y1 + y2) / 2,
  labels = as.character(sum(table.1[, 2])),
  adj = 0.5
)
...
table.2 <-
  t(table(
    ...
    c(doi_unique$is_oa, my_dois_works$is_oa)
  ))
table.2 <- table.2[order(rownames(table.2)), ]
rownames(table.2) <- c("Open acess", "Paywall")
...
if(sum(c(doi_unique$citations, my_dois_works$citations)) == 0){
  ...
  table.3 <-
    t(table(
      c(doi_unique$published_on, my_dois_works$published_on),
      c(doi_unique$citations, my_dois_works$citations)
    ))
  table.3 <- table.3[order(rownames(table.3)),]
  table.3 <- colSums(table.3)
}
rownames(table.3) <- c("Citações")
...
```{r sintese-visibilidade-carousel, eval = has.producao}
# carousel
bs_carousel(id = "visibilidade-carousel",
            use_indicators = FALSE,
            use_controls = TRUE) %>%
  bs_append(content = bs_carousel_image(src = "index_files/figure-html/sintese-visibilidade-1.png")) %>%
  bs_append(content = bs_carousel_image(src = "index_files/figure-html/sintese-visibilidade-2.png")) %>%
  bs_append(content = bs_carousel_image(src = "index_files/figure-html/sintese-visibilidade-3.png")) %>%
  bs_append(content = bs_carousel_image(src = "index_files/figure-html/sintese-visibilidade-4.png"))

cat('\n\n')
cat(
  "[**Altmetric**](https://www.altmetric.com), [**CrossRef**](https://www.crossref.org), **PPG**"
)
cat('^[Produções com DOI apenas; a produção pode estar subsestimada pois produtos sem DOI não estão apresentados.]')
cat('\n')
cat('^[O Altmetric score no donut representa a soma total do Altmetric score de cada produção.]')
```
...
g2 <- ggdraw(g) +
  theme(plot.background = element_rect(fill = main.color, color = NA))
