...
# plot area setup
layout(matrix(c(1, 2, 3, 1, 4, 5, 6, 7, 8),
              ncol = 3,
              byrow = TRUE))
...
part.ext.nac <-
  sucupira.raw$`Nome do Participante`[sucupira.raw$`País da Instituição de Origem` == "Brasil"]
part.ext.inter <-
  sucupira.raw$`Nome do Participante`[sucupira.raw$`País da Instituição de Origem` != "Brasil"]

IES.nac <-
  sucupira.raw$`Instituição de Origem`[sucupira.raw$`País da Instituição de Origem` == "Brasil"]
IES.inter <-
  sucupira.raw$`Instituição de Origem`[sucupira.raw$`País da Instituição de Origem` != "Brasil"]
...
top <- min(length(unique(IES.nac)), 10)
data.nac <- na.omit(data.frame(name = names(sort(table(IES.nac), decreasing = TRUE)[1:top]),
                       value = as.numeric(sort(table(IES.nac), decreasing = TRUE)[1:top])))

# break vector names in 2 lines
text.2.break <- data.nac$name
source("Scripts/line_break.R")
...
top <- min(length(unique(IES.inter)), 10)
data.inter <- na.omit(data.frame(name = names(sort(table(IES.inter), decreasing = TRUE)[1:top]),
                         value = as.numeric(sort(table(IES.inter), decreasing = TRUE)[1:top])))
...
top <- min(length(unique(mundo)), 10)
data.pais <- na.omit(data.frame(name = names(sort(table(mundo), decreasing = TRUE)[1:top]),
                         value = as.numeric(sort(table(mundo), decreasing = TRUE)[1:top])))
...
# plot area setup
layout(matrix(c(1, 1, 2, 3, 4, 4, 5, 6),
              ncol = 2,
              byrow = TRUE))

# plot 5 (summary table in plot)
for (i in 1:ncol(summ.table.ies)) {
  plot(
    c(0, 1),
    c(0, 1),
    ann = F,
  ...
  text(
    x = 0.5,
    y = 0.6,
    labels = as.character(summ.table.ies[i]),
    cex = 4.2,
    col = brewer.pal(n = 9, name = "Set3")[3]
  )
  text(
    x = 0.5,
    y = 0.2,
    labels = colnames(summ.table.ies)[i],
    cex = 2,
    col = brewer.pal(n = 9, name = "Set3")[2]
  )
  box(lty = "solid",
