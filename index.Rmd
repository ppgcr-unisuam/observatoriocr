...
---
title: "OBSERVATÓRIO `r sigla.PPG`"
---
...
<img src="PPG/Images/logo-IES.png" align="center"/>

<br>

<p style="font-size:1.5em"> O **Observatório `r toupper(sigla.PPG)`** é uma iniciativa do **Programa de Pós-graduação em `r nome.PPG`** do [`r toTitleCase(as.character(nome.IES))`](`r site.PPG`) que envolve o registro, o acompanhamento e a análise agregada e estruturada de dados para divulgação transparente de suas atividades, planejamentos, ações e impactos, tornando-os acessíveis à sociedade.</p>
...
```{r sintese-coerencia, include = FALSE}
# projetos
...
for(i in 1:length(projetos.id)){
  matches <- sucupira.raw[sucupira.raw$`Identificador do Projeto` == projetos.id[i], ]
  if(dim(matches)[1] == 1){
    sucupira <- bind_rows(sucupira, matches)
  } else {
    keep <- matches[matches$Situação == "CONCLUÍDO" | !is.na(matches$`Data de Fim da Linha`), ]
...
alluv.data <- alluv.data[!is.na(alluv.data$sucupira.raw..Área.de.Concentração.) & !is.na(alluv.data$sucupira.raw..Linha.de.Pesquisa.), ]
...
for(i in 1:dim(summ.AC.LP)[1]){
  if(as.character(summ.AC.LP$Inicio[i]) > min(as.character(summ.AC.LP$Inicio))){
    extra <- rbind(extra, c(as.character(summ.AC.LP$AC[i]), as.character(summ.AC.LP$LP[i]), min(as.character(summ.AC.LP$Inicio)), as.character(summ.AC.LP$Inicio[i]), as.character(summ.AC.LP$Freq[i])))
  }
}
...
alluv.data$sucupira.raw..Nome.do.Projeto.de.Pesquisa. <- as.numeric(as.factor(alluv.data$sucupira.raw..Nome.do.Projeto.de.Pesquisa.))
...
sn.title <- "Área de Concentração - Linha de Pesquisa"
...
sn <- sankeyNetwork(Links = links, Nodes = nodes,
  ...
  htmlwidgets::prependContent(
    htmltools::tags$h1(style = "position:absolute; left:50%; transform:translateX(-50%); background-color:#2C3E50; font-size:20px; color:white; line-height:normal;", sn.title)
  ) %>%
  htmlwidgets::prependContent(
...
# save it in html
saveWidget(sn, file.path(dir.path, "tmp.html"), selfcontained = F)
...
```{r sintese-projetos, include = FALSE}
# projetos - membros
...
```{r sintese-financiadores, include = FALSE}
options("scipen" = 100)
...
```{r sintese-financiadores-carousel, eval = has.file}
# this chunk requires additional data that may  be available
has.file <-
  !is.error(try(financiadores.raw <-
                  read_excel("PPG/Financiadores.xlsx", sheet = "financiadores"),
...
try(bs_carousel(id = "financiadores-carousel",
                use_indicators = FALSE,
                use_controls = TRUE) %>%
      bs_append(id = "financiadores-carousel", content = bs_carousel_image(src = "index_files/figure-html/sintese-financiadores-1.png")) %>%
      bs_append(id = "financiadores-carousel", content = bs_carousel_image(src = "index_files/figure-html/sintese-financiadores-2.png")) %>%
      bs_append(id = "financiadores-carousel", content = bs_carousel_image(src = "index_files/figure-html/sintese-financiadores-3.png")) %>%
      bs_append(id = "financiadores-carousel", content = bs_carousel_image(src = "index_files/figure-html/sintese-financiadores-4.png")) %>%
      bs_append(id = "financiadores-carousel", content = bs_carousel_image(src = "index_files/figure-html/sintese-financiadores-5.png")) %>%
      bs_append(id = "financiadores-carousel", content = bs_carousel_image(src = "index_files/figure-html/sintese-financiadores-6.png")),
    silent = TRUE)
...
```{r sintese-producao, include = FALSE}
# read data
artigos <-
...
artigos <- rbind(artigos[, 1:11], resumos[, 1:11], eventos[, 1:11])
...
table.4 <- data.frame(artigos$Revista..digite.ou.selecione.na.lista., SJR)
...
legend(
  ...
  fill = brewer.pal(n = 9, name = "Set3")[1:2],
  text.col = "white",
  box.lwd = 0,
  bty = "n",
  horiz = TRUE,
...
```{r sintese-pessoal, include = FALSE}
# reading PPG data
discentes.raw <-
