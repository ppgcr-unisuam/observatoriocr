...
linhas <-
  as.character(na.exclude(unique(alluv.data$sucupira.raw..Linha.de.Pesquisa.)))
...
vioplot(
  as.numeric(table.3[,2]) ~ as.numeric(table.3[,1]),
  xlab = NA,
  ylab = "SJR"
)
...
cursos <- levels(as.factor(discentes$Curso))
...
table.2.msc <- data.frame(mestres$`Ano Titulação`, as.numeric(mestres$`Tempo de Curso (meses)`))
table.2.dsc <- data.frame(doutores$`Ano Titulação`, as.numeric(doutores$`Tempo de Curso (meses)`))
...
vioplot(
  ...
  ylim = c(min(as.numeric(table.2.msc[,2]), as.numeric(table.2.dsc[,2])), max(as.numeric(table.2.msc[,2]), as.numeric(table.2.dsc[,2]))),
  ylab = "Meses"
)
vioplot(
...
[**Discentes**](pessoas.html#discentes)
...
[**Disciplinas oferecidas**](programa.html#estrutura-curricular)
