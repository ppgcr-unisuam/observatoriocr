...
```{r try-financiadores}
has.financiadores <-
  !is.error(try(read_excel("PPG/Financiadores.xlsx", sheet = "financiadores"),
                silent = TRUE)
  )
```
...
```{r try-producao}
has.producao <-
  !is.error(try(read_excel(paste0("../", PPG.folder, "/Produção.xlsx"), sheet = "PI.completos"), silent = TRUE))
```
...
artigos <-
  data.frame(read_excel(
    paste0("../", PPG.folder, "/Produção.xlsx"),
    sheet = "PI.completos",
    col_types = c("text")
  ))
...
```{r try-discentes}
has.discentes <-
  !is.error(try(read_excel(paste0("../", PPG.folder, "/Discentes.xlsx"), sheet = "DATA.discentes"),
                silent = TRUE))
```
...
discentes.raw <-
  read_excel(paste0("../", PPG.folder, "/Discentes.xlsx"), sheet = "DATA.discentes")
