...
title(
  main = paste0("Top-", top, " periódicos"),
  outer = TRUE,
  cex.main = 1.7
)
options(digits = 2)
...
qualis.all <- table(qualis$ESTRATO)/length(qualis$ESTRATO)*100
qualis.PPG <- table(artigos$WebQualis)/length(artigos$WebQualis)*100
...
barplot(
  ...
  xlab = "%",
  ylab = "Estratos",
  xlim = c(-max(qualis.all, qualis.PPG), max(qualis.all, qualis.PPG)),
  ylim = c(0,length(unique(qualis.all))+3),
