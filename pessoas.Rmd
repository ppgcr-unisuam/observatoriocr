...
## **Formação continuada** {#formacao-continuada .tabset}
...
for (i in 1:length(ID)) {
  ...
  table[i, "Iniciação científica"] <- min(data$`Data Matrícula`[data$Nível == "Bacharelado"], na.rm = TRUE)
  table[i, "Iniciação científica"] <- format(as.Date(table[i, "Iniciação científica"], format = "%d/%m/%Y"), "%Y")
...
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(table.continuada, title = "Impactos - Formação continuada")
