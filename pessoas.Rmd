...
## **Coordenação** {#coordenacao .tabset}
...
table.coord <- table.coord[order(table.coord$'Vigência', decreasing = FALSE),]
...
portarias.raw <- data.frame()
# Read Excel data
try(portarias.raw <- readxl::read_excel("PPG/Coordenação.xlsx", sheet = "Portarias"), silent = TRUE)
if(!sjmisc::is_empty(portarias.raw)){
  cat('\n\n<!-- -->\n\n')
  cat('### **Portarias**')
  cat('\n\n<!-- -->\n\n')
  
  # replace the link by a tag
  Abrir <- matrix(NA, nrow = dim(portarias.raw)[1])
  colnames(Abrir) <- "Abrir"
  # add hyperlinks
  for (i in 1:dim(portarias.raw)[1]) {
    if (!is.na(portarias.raw[i, ncol(portarias.raw)])) {
      Abrir[i] <-
        paste0(
          '<a href="',
          portarias.raw[i, ncol(portarias.raw)],
          '" target="_blank"',
          '>',
          # fontawesome icon
          fontawesome::fa("up-right-from-square"),
  ...
  portarias <- cbind(portarias.raw[, -ncol(portarias.raw)], Abrir)
  
  # print table
  source("Scripts/table-with-buttons.R", local = knitr::knit_global())
  cat(knitr::knit_print(create_dt(portarias, title = "Pessoas - Portarias")))
}
```
...
sucupira <- sucupira.list[as.character(periodo)] %>% plyr::compact()
sucupira <- data.frame(do.call(dplyr::bind_rows, sucupira))
names(sucupira) <- names(sucupira.list[[as.character(max(as.numeric(anos)))]])
...
sucupira <- data.frame(do.call(dplyr::bind_rows, sucupira))
names(sucupira) <-
  names(sucupira.list[[as.character(max(as.numeric(anos)))]])
...
```{r ids-externos-docentes, eval = all(has.sucupira.files, curl::has_internet())}
# Read Excel data
sheet <- "Docentes"
source("Scripts/read-xlsx-sucupira.R", local = knitr::knit_global())
...
sucupira <- sucupira[is.na(sucupira$`Data de Desligamento`),]
...
if (file.exists(file.path("cache", "pessoas", "external-id.Rds"))) {
  external.ids <- readRDS(file.path("cache", "pessoas", "external-id.Rds"))
} else {
  dir.create(file.path("cache", "pessoas"), showWarnings = FALSE, recursive = TRUE)
  source("Scripts/orcid-external-identifiers.R", local = knitr::knit_global())
  external.ids <- get_external_ids(my_orcid)
  
  saveRDS(external.ids, file.path("cache", "pessoas", "external-id.Rds"))
}
...
```{r discentes-matriculados, eval = has.sucupira.files}
# get years
source("Scripts/years-sucupira.R", local = knitr::knit_global())
...
# periodo max
periodo <- as.numeric(max(as.numeric(names(sucupira.list))))

# get egressos for the current period
discentes <- sucupira.list[as.character(periodo)] %>%
  plyr::compact()
discentes <- data.frame(do.call(dplyr::bind_rows, discentes))
try(names(discentes) <- names(sucupira.list[[as.character(max(as.numeric(anos)))]]), silent = TRUE)
...
discentes <- discentes[!is.na(discentes$`Identificador da Pessoa`), ]
...
### **Distribuição nacional** {#discentes-nacional}

```{r discentes-periodo, eval = has.discentes}
discentes.raw <-
  readxl::read_excel(
    "PPG/Discentes.xlsx",
    sheet = "Discentes",
...
discentes <- discentes.raw[is.na(discentes.raw$'Data Situação'), ]
discentes$`Ano Matrícula` <- as.numeric(format(discentes$`Data Matrícula`, format = "%Y"))

# plot Brazil states per period
periodos <-
  rbind(cbind(2010, 2012), cbind(2013, 2016), cbind(2017, 2020), cbind(2021, 2024), cbind(2025, 2028))
colnames(periodos) <- c("Início", "Fim")
rownames(periodos) <- c("Trienal", "Quadrienal", "Quadrienal", "Quadrienal", "Quadrienal")
...
for (j in 1:dim(periodos)[1]) {
  discentes.periodo <-
    discentes[discentes$`Ano Matrícula` >= periodos[j, 1] &
                discentes$`Ano Matrícula` <= periodos[j, 2], ]
  states$discentes <- rep(NA, dim(states)[1])
  UF <- as.data.frame(table(discentes.periodo$UF))
...
try(names(egressos) <- names(sucupira.list[[as.character(max(as.numeric(anos)))]]), silent = TRUE)
...
try(egressos <- egressos %>% dplyr::select("Nível Discente", "Nome Discente", "Data Situação", "Nome do Orientador", "País"), silent = TRUE)
...
# order by 3rd and 2 columns
egressos <- egressos[order(egressos$'Data Situação', egressos$'Nome'), ]
...
# periodo
n.end <- max(as.numeric(anos)) - 1
n.start <- max(as.numeric(anos)) - 5
# egressos dos últimos 5 anos
periodo <- n.start:n.end
periodo <- periodo[periodo %in% anos]
...
# periodo
n.end <- max(as.numeric(anos)) - 6
n.start <- max(as.numeric(anos)) - 10
# egressos dos últimos 5 anos
periodo <- n.start:n.end
periodo <- periodo[periodo %in% anos]
...
### **10 a 15 anos** {#egressos-15}

```{r historico-egressos-15, eval = has.sucupira.files}
# get years
source("Scripts/years-sucupira.R", local = knitr::knit_global())

# periodo
n.end <- max(as.numeric(anos)) - 11
n.start <- max(as.numeric(anos)) - 15
# egressos dos últimos 5 anos
periodo <- n.start:n.end
periodo <- periodo[periodo %in% anos]
...
for (j in 1:dim(periodos)[1]) {
  egressos.periodo <-
    egressos[egressos$`Ano Titulação` >= periodos[j, 1] &
                egressos$`Ano Titulação` <= periodos[j, 2], ]
  states$egressos <- rep(NA, dim(states)[1])
  UF <- as.data.frame(table(egressos.periodo$UF))
  for (i in 1:length(UF$Var1)) {
...
```{r discentes-encerrados, eval = has.sucupira.files}
# discentes
sheet <- "Discentes - Orientadores"
source("Scripts/read-xlsx-sucupira.R", local = knitr::knit_global())
...
# dplyr::select students that are not matriculated or have already graduated
discentes <- discentes[discentes$`Situação Discente` %in% c("DESLIGADO", "ABANDONOU"), ]
...
# get years
discentes$'Ano de Encerramento' <- substr(as.Date(discentes$`Data Situação`, format = "%d/%m/%Y"), 1, 4)
...
# drop columns
discentes <- discentes %>% dplyr::select("Nível Discente", "Nome Discente", "Ano de Matrícula", "Ano de Encerramento", "Tempo em curso", "Situação Discente", "País")
...
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(discentes, title = "Pessoas - Discentes - Encerrados")
...
```{r externos, eval = has.sucupira.files}
