...
```{r preprints-metrics, cache = TRUE}
# read data
preprints <-
  data.frame(read_excel(
    "../PPGCR Unisuam/Produção.xlsx",
...
if (dim(dois)[1] == 0) {
  ...
  my_dois_works_preprints$published_on <-
    format(as.Date(my_dois_works_preprints$published.online), format = "%Y")
}
```

```{r livros-capitulos-metrics, cache = TRUE}
# read data
livros_capitulos <-
  data.frame(read_excel(
    "../PPGCR Unisuam/Produção.xlsx",
...
if (dim(dois)[1] == 0) {
  ...
  my_dois_works_livros_capitulos$published_on <-
    format(as.Date(my_dois_works_livros_capitulos$published.online), format = "%Y")
}
```
...
years <- na.omit(rev(unique(c(doi_unique$published_on, my_dois_works$published_on))))
...
if (is.null(dim(doi_unique)[1]) & is.null(dim(my_dois_works)[1])) {
  ...
  # print tab header
    ...
    table.with.badges(
      ...
      show.Qualis = TRUE,
      tryCatch(
        doi_unique[doi_unique$published_on == years[i], ],
        error = function(e) {
        }
      ),
      tryCatch(
        my_dois_works[my_dois_works$published_on == years[i], ],
        error = function(e) {
        }
      ),
      scimago,
