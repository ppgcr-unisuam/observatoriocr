...
# sort by Área de concentração, Linha de pesquisa, Projeto de pesquisa
...
if (dim(financiadores.all)[1] != 0) {
  ...
  # revise foreing funders
  table.financiadores$`Financiador Estrangeiro`[financiadores.all$`Tipo de Documento do Financiador` == "Pessoa Jurídica Estrangeira"] <- "Sim"
...
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(financiadores, title = "Programa - Financiadores - Detalhamento")
...
for (j in 1:length(abas)) {
  ...
  # order by 1st column
  convenios <- convenios[order(convenios[, 1], decreasing = TRUE),]
...
for (j in 1:length(abas)) {
  ...
  # order by 1st column
  cooperacoes <- cooperacoes[order(cooperacoes[, 1], decreasing = TRUE),]
