...
create_dt(area.capes, title = "Áreas de Concentração")
...
# print tables
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(area.ativas, title = "Áreas de Concentração")
...
# print tables
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(areas.inativas, title = "Áreas de Concentração (inativas)")
...
# print tables
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(linhas.atual, title = "Linhas de Pesquisa")
...
# print tables
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(linhas.inativas, title = "Linhas de Pesquisa (inativas)")
...
```{r projetos-de-pesquisa-em-andamento-lista, eval = has.sucupira.files}
table.projetos <-
  data.frame(
...
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(table.projetos, title = "Projetos de Pesquisa - Em andamento")
...
```{r projetos-de-pesquisa-encerrados-lista, eval = has.sucupira.files}
table.projetos <-
  data.frame(
    projetos$`Área de Concentração`,
    projetos$`Linha de Pesquisa`,
...
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(table.projetos, title = "Projetos de Pesquisa - Encerrados")
...
ids <- order(bibliografia.raw$'Disciplina', decreasing = FALSE)
bibliografia.raw <- bibliografia.raw[ids,]
...
for (j in 1:length(abas)) {
  ...
  ids <- order(calendarios[, 1], decreasing = TRUE)
  calendarios <- calendarios[ids,]

  # print table
  if (dim(calendarios)[1] != 0) {
    source("Scripts/table-with-buttons.R", local = knitr::knit_global())
    cat(knitr::knit_print(create_dt(calendarios, title = "Calendários")))
  } else {
    cat("*Sem dados para exibir*")
  }
...
for (j in 1:length(abas)) {
  ...
  ids <- order(editais[, 1], decreasing = TRUE)
  editais <- editais[ids,]

  # print table
  if (dim(editais)[1] != 0) {
    source("Scripts/table-with-buttons.R", local = knitr::knit_global())
    cat(knitr::knit_print(create_dt(editais, title = "Editais")))
  } else {
    cat("*Sem dados para exibir*")
  }
...
for (j in 1:length(abas)) {
  ...
  # print table
  if (dim(infraestrutura.raw)[1] != 0) {
    source("Scripts/table-with-buttons.R", local = knitr::knit_global())
    cat(knitr::knit_print(create_dt(infraestrutura.raw, title = "Infraestrutura")))
  } else {
    cat("*Sem dados para exibir*")
  }
...
for (j in 1:length(abas)) {
  ...
  for (i in 1:dim(internacional.raw)[1]) {
    if (!is.na(internacional.raw[i, 4])) {
      Abrir[i] <-
        paste0('<a href="',
               internacional.raw[i, 4],
               '" target="_blank"',
               '>',
               "↗️" ,
               "</a>",
  ...
  internacional <- cbind(internacional.raw[, -4], Abrir)
...
```{r financiadores-sucupira-pp, eval = has.sucupira.files}
# projetos - membros
...
if (dim(financiadores.all)[1] != 0) {
  ...
  table.financiadores <-
    data.frame(
      financiadores.all$`Nome do Financiador`,
      financiadores.all$`Natureza do Financiador`,
      financiadores.all$`Data Início Financiador`,
      financiadores.all$`Data Fim Financiador`
    )
  ...
  colnames(table.financiadores) <-
    c("Agência",
      ...
      "Fim")
  source("Scripts/table-with-buttons.R", local = knitr::knit_global())
  create_dt(table.financiadores, title = "Financiadores - Projetos de Pesquisa")
} else {
  cat('\n\n')
...
if (dim(financiadores)[1] != 0) {
  ...
  table.financiadores <-
    data.frame(
      financiadores.all$`Nome do Financiador`,
      financiadores.all$`Nome do Programa de Fomento`,
      financiadores.all$`Data início da orientação`,
      financiadores.all$`Data fim da orientação`
    )
  table.financiadores <- table.financiadores[order(financiadores.all[, 1]), ]
  # print tables
  table.financiadores[, 1] <-
  ...
  table.financiadores[, 2] <-
    as.vector(unlist(lapply(table.financiadores[, 2], FUN = stringr::str_to_sentence)))
  table.financiadores[, 3] <-
    as.vector(unlist(lapply(table.financiadores[, 3], FUN = stringr::str_to_title)))
...
for (j in 1:length(abas)) {
  ...
  # print table
  if (dim(bolsas.raw)[1] != 0) {
    source("Scripts/table-with-buttons.R", local = knitr::knit_global())
    cat(knitr::knit_print(create_dt(bolsas.raw, title = "Bolsas de estudo")))
  } else {
    cat("*Sem dados para exibir*")
  }
